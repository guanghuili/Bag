...
-(void)awakeFromNib
{
    ...
    _swipeContentViewX = _swipeContentView.oc_left;
...
-(void)pullbackSwipeContentViewAnimationd:(BOOL)animation completion:(void (^)(void))completion
{
    ...
    if (animation)
    {
        [UIView animateWithDuration:0.3 animations:^{
            ...
            _swipeContentView.oc_left = _swipeContentViewX;
...
//    }
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        _swipeContentView.oc_left += translation.x;
        [recognizer setTranslation:CGPointZero inView:self.superview];
        
        _swipeLeftEditView.hidden = _swipeContentView.oc_left < _swipeLeftEditView.oc_left;
        _swipeRightEditView.hidden = _swipeContentView.oc_left > _swipeLeftEditView.oc_left;
        
    } else if(recognizer.state == UIGestureRecognizerStateEnded) {
    
        
        
        if (_swipeLeftEditView && _swipeContentView.oc_left  >= _swipeLeftEditView.oc_right / 2) {
            [UIView beginAnimations:@"AnimationMove" context:NULL];
    
                _swipeContentView.oc_left = _swipeLeftEditView.oc_right;
            
            [UIView commitAnimations];
        
            [self setEditing:YES];
            
        } else if (_swipeRightEditView && _swipeContentView.oc_right <= _swipeRightEditView.oc_right - _swipeRightEditView.oc_width/2) {
            
            [UIView beginAnimations:@"AnimationMove" context:NULL];
            
                _swipeContentView.oc_right = _swipeRightEditView.oc_left;
